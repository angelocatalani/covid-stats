...
def retrieve_country_data(
    ...
    try:
        ...
        logger.warning(
            f"network error retrieving country: {country} from {start_date_string} to {end_date_string}"
        )
        raise ApiError(
            f"network error retrieving country: {country} from {start_date_string} to {end_date_string}"
        )
    ...
    if "success" in response and not response["success"]:
        logger.warning(
            f"{response} retrieving data for country: {country} from {start_date_string} to {end_date_string}"
        )
...
def retrieve_all_countries_data(
    ...
    for country in countries:
        logger.info(
            f"start retrieving data for {i}/{all_countries}-th country: {country} from {start_date} to {end_date}"
        )
